...
test('database policy rejects over-budget and duplicate approvals atomically', { skip: !enabled }, async () => {
  ...
  try {
    await db.begin(async (tx) => {
      ...
      const [user] = await tx`insert into users (google_sub, email, name, wallet_scope) values (${`integration-${suffix}`}, ${`integration-${suffix}@example.test`}, 'Integration test', ${`integration-scope-${suffix}`}) returning id`;
      ...
      await assert.rejects(
        () => tx.savepoint(async (savepoint) => savepoint`select * from reserve_purchase_intent(${duplicate.id}, ${user.id}, ${randomUUID()})`),
        /no longer valid/i,
      );
